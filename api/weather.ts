...
// 앱이 보낸 GPS 좌표(lat/lon) 또는 IP 기반 대략 위치(Vercel geo 헤더)
//   → 기상청 격자 변환 → 초단기실황(PTY) 조회.
// 좌표는 요청 처리 중에만 쓰고 저장하지 않으며, 기상청엔 5km 격자값만 나간다.
// 앱이 COARSE 위치 권한을 쓰므로 Play 데이터안전 폼에 "대략적 위치 = 수집"으로
// 신고돼 있다(선택 항목 / 일시적 처리). 상세: STORE_LISTING.md §2.2
