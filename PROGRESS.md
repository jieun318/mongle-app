...
0015 delete_my_account — 아바타 Storage 정리   ← 프로덕션 적용 완료(2026-08-09)
```
...
- [x] **런처 이름** `app.json` `expo.name`: `mongle` → `몽글` (2026-08-05).
      ...
      → **versionCode 9 재빌드** (7번 AAB 는 아직 `mongle` 라벨)
- [x] 계정 삭제 안내 인앱 링크 — 마이페이지 푸터 + `privacy.tsx` 6조
...
- [x] 프로덕션 AAB 빌드 (2026-08-09, **versionCode 11** / `1b4ecd8`) — §6.1 참고.
      9번 폐기: 앱 코드는 동일하나 미사용 권한(마이크·오버레이 등) 미정리 상태
- [x] **0015 마이그레이션 프로덕션 적용** (2026-08-09) — 계정 삭제 시 아바타
      Storage 객체까지 삭제. `/account-deletion` 안내와 동작 일치
- [x] **심사용 카카오 테스트 계정** 발급 (2026-08-09) → 콘솔 앱 액세스 권한 입력
...
| 7 | `f2c99c4` | 2026-07-29 | 구 1차 후보 — **폐기**(mood_tags 크래시 미포함 + 런처명 영문) |
| 8 | — | — | 번호만 소비 (원격 카운터 기준, 빌드 기록 없음) |
| 9 | `1a7dd3f` | 2026-08-05 | 폐기 — 빌드 `4bca513d`. 앱 코드는 완전했으나 미사용 권한 미정리 |
| 10 | — | — | 번호만 소비 (원격 카운터 기준, 빌드 기록 없음) |
| **11** | **`1b4ecd8`** | **2026-08-09** | **1차 릴리스 후보** — 빌드 `5827d89e` |
